...
import { Plus, MessageSquare, Trash2, Home, PanelLeftClose, ChevronDown, Settings, LogOut } from 'lucide-react'
...
export default function ChatSidebar({
  chats, currentChatId, isSidebarOpen, isDesktopSidebarOpen, deleteConfirmId, sidebarAccountOpen,
  userName, userPlan, userAvatar, onSelectChat, onNewChat, onDeleteClick, onToggleSidebar,
  onToggleDesktopSidebar, onToggleAccount, onLogout, t,
}: Props) {
  return (
    <>
      <aside className={`${isDesktopSidebarOpen ? 'w-80' : 'w-0 overflow-hidden'} bg-white border-r border-gray-200 flex-col hidden lg:flex flex-shrink-0 transition-all duration-300`}>
        ...
        <div className="flex-1 overflow-y-auto px-3 space-y-1 min-w-80">
        ...
        <div className="p-4 border-t border-gray-200">
          ...
          {sidebarAccountOpen && (
            <div className="mt-2 space-y-1">
              ...
              <button onClick={onLogout} className="flex items-center gap-3 px-4 py-2 rounded-xl text-sm text-red-600 hover:bg-red-50 transition-colors w-full">
                <LogOut className="w-4 h-4" />{t('chat.logout')}
              </button>
            </div>
          )}
        </div>
